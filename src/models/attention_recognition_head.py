...
class AttentionRecognitionHead(nn.Module):
    ...
    def forward(self, x):
        ...
        # Initialize decoder's initial state
        state = torch.zeros(1, batch_size, self.sDim, device=x.device)
        outputs = []

        for i in range(max(lengths)):
            y_prev = torch.full((batch_size, ), self.num_classes, dtype=torch.float32, device=x.device) if i==0 else targets[:,i-1]
    ...
    def sample(self, x):
        ...
        for i in range(self.max_len_labels):
            y_prev = torch.full((batch_size, ), self.num_classes, dtype=torch.float32, device=x.device) if i==0 else predicted  
            output, state = self.decoder(x, state, y_prev)
    ...
    def beam_search(self, x, beam_width, eos):
        ...
        state = torch.zeros(1, batch_size * beam_width, self.sDim, device=x.device)
        ...
        y_prev = torch.full((batch_size* beam_width, ), self.num_classes, dtype=torch.float32, device=x.device)
        ...
        for i in range(self.max_len_labels):
            ...
            sequence_scores = _inflate(sequence_scores, self.num_classes, 1).to(x.device)
            ...
            predecessors = (candidates // self.num_classes + pos_index.expand_as(candidates).to(x.device)).view(batch_size * beam_width, 1)
            state = state.index_select(1, predecessors.to(x.device).squeeze())
        ...
        t_predecessors = (sorted_idx + pos_index.expand_as(sorted_idx).to(x.device)).view(batch_size * beam_width)
        while t >= 0:
            ...
            eos_indices = torch.nonzero( stored_emitted_symbols[t].eq(eos), as_tuple=False)
        ...
        re_sorted_idx = (re_sorted_idx + pos_index.expand_as(re_sorted_idx).to(x.device)).view(batch_size*beam_width)
